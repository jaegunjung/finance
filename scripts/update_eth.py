"""
Update crypto/data/eth_daily.csv with new ETH price data from CoinGecko.
...
def main():
    ...
    # If CSV doesn't have full history (starts after genesis), fetch full history
    needs_full_history = first_date_str > ETH_GENESIS
    ...
    # If full history fetch, also write historical rows not yet in CSV
    if needs_full_history:
        historical_rows = []
        for ts, price in data['prices']:
            date_str = datetime.fromtimestamp(ts / 1000, tz=timezone.utc).strftime('%Y-%m-%d')
