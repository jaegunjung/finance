...
date: 2026-06-07
categories: crypto
tags: [bitcoin, halving, cycle-analysis, log-scale, BTC]
excerpt: "Four halving cycles mapped on a log scale — what history says about where Bitcoin stands today."
excerpt_en: "Four halving cycles mapped on a log scale — what history says about where Bitcoin stands today."
excerpt_ko: "로그 스케일로 본 4번의 반감기 사이클 — 지금 비트코인이 사이클상 어디에 있는지 데이터로 진단한다."
---
...
사이클 1을 예외로 두면, 사이클 2와 3은 일관된 패턴을 보인다: 반감기 후 12~18개월 구간은 **변동성이 높지만 상향 바이어스**가 존재하는 구간이다 — 사이클의 끝이 아니다. 과거 3번의 사이클에서 진짜 고점은 모두 **반감기 후 16~22개월** 사이에 나타났다.
...
**MVRV Z-Score**는 시장가 대 실현가(시장이 실제로 지불한 가격) 비율을 측정한다. 1 이하는 역사적 저점과 겹쳤다. 7 이상은 고점과 겹쳤다. 사이클 중간 조정은 통상 2~3 구간을 터치한다.

**Puell Multiple**은 일일 채굴자 수익을 365일 이동평균 대비로 추적한다. 채굴자들이 수익 압박을 받을 때(Puell < 0.5), 이는 역사적으로 저점과 동시에 혹은 직전에 나타났다. 채굴자 항복은 공급 측 약한 손을 청산시킨다.
